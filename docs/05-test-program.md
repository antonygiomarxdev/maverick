...
4. **Fault-injection** — SQLite `SQLITE_BUSY` / concurrent writer covered in `tests/persistence_sqlite.rs` (`sqlite_concurrent_transaction_waits_on_busy_then_succeeds`). Radio transport: timeout + circuit breaker + UDP happy path in `cargo test -p maverick-adapter-radio-udp`; cross-crate wiring in `tests/radio_transport_resilience.rs`. Burst ingress and broader fault matrix remain queued.
...
- Evidence log: `docs/slice-2-evidence.md`, `docs/slice-3-evidence.md` (transport).
