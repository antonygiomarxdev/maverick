...
| Runtime inbound wiring | Added `maverick-edge radio ingest-once --bind --timeout-ms` to bind UDP, receive one packet, parse GWMP, and execute `IngestUplink` using `SqlitePersistence` repositories + `LoRaWAN10xClassA` protocol module. |
...
- `radio ingest-once` is intentionally a one-shot diagnostic path; a long-running supervised ingest loop remains future work.
