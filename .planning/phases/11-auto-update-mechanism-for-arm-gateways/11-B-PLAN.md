...
type: execute
files_modified:
  - crates/maverick-runtime-edge/src/update.rs (version checking + download logic)
  - crates/maverick-runtime-edge/src/update/version.rs (new file)
  - crates/maverick-runtime-edge/src/update/download.rs (new file)
autonomous: true
requirements: []
---

<objective>
Implement Rust update logic: version checking, HTTPS download, atomic binary replacement, and backup management. The shell script (Plan A) calls into this logic for the actual version check and download.

**Out of scope:** CLI commands (Plan C), systemd timer setup (Plan A)
</objective>

<read_first>
- .planning/phases/11-auto-update-mechanism-for-arm-gateways/11-CONTEXT.md (update decisions)
- .planning/phases/10-libloragw-spi-integration/10-CONTEXT.md (Phase 10 context for Rust patterns)
- crates/maverick-runtime-edge/src/main.rs (existing CLI structure)
</read_first>

<action>
Create `crates/maverick-runtime-edge/src/update.rs` — Update module:
...
impl UpdateConfig {
    ...
    pub fn load() -> Result<Self, UpdateError> {
        ...
        // Simple TOML parsing for [update] section
    ...
    pub fn check_release_update(&self) -> Result<Option<String>, UpdateError> {
        let release_url = self.release_url.as_ref()
    ...
    pub fn apply_release_update(&self, new_version: &str) -> Result<(), UpdateError> {
        ...
        let download_path = self.download_dir.join(format!("maverick-edge-{}", new_version));
        ...
        // Atomic replace: copy to .new, then mv
    ...
    pub fn apply_dev_update(&self, repo_path: &Path) -> Result<(), UpdateError> {
        ...
        if !output.status.success() {
            return Err(UpdateError::Build(String::from_utf8_lossy(&output.stderr).to_string()));
        }
    ...
    fn cleanup_old_backups(&self) -> Result<(), UpdateError> {
        let entries: Vec<_> = std::fs::read_dir(&self.backup_dir)
            ...
            .filter_map(|e| e.ok())
            .filter(|e| {
                e.file_name().to_string_lossy().starts_with("maverick-edge-")
            })
            .collect();
        ...
        // Remove oldest entries beyond the last 2
...
Create `crates/maverick-runtime-edge/src/update/version.rs`:
...
pub fn fetch_remote_version(version_url: &str, insecure: bool) -> Result<String, UpdateError> {
    let output = if insecure {
        ...
        std::process::Command::new("curl")
            .args(["-s", "-L", "-f", "--cacert", "/etc/ssl/certs/ca-certificates.crt", version_url])
            .output()
    ...
    // Basic validation: must be semver-like
    if version.is_empty() || !version.matches('.').count() >= 1 {
        return Err(UpdateError::Version(format!("Invalid version string: {}", version)));
    }
...
Create `crates/maverick-runtime-edge/src/update/download.rs`:
...
pub fn download_file(url: &str, dest: &std::path::Path, insecure: bool) -> Result<(), UpdateError> {
    ...
    if metadata.len() == 0 {
        return Err(UpdateError::Download("Downloaded file is empty".to_string()));
    }
...
Add module to `crates/maverick-runtime-edge/src/update.rs`:
```rust
pub mod version;
pub mod download;
```
...
<acceptance_criteria>
- `update.rs` implements `UpdateConfig::load()` from TOML
- `UpdateConfig::check_release_update()` fetches version.txt and compares versions
- `UpdateConfig::apply_release_update()` downloads binary, backs up current, atomically replaces
- `version::fetch_remote_version()` uses curl with appropriate flags (insecure mode available)
- `download::download_file()` verifies download succeeded and file is non-empty
- Backup cleanup keeps last 2 backups
- Error types use thiserror::Error derive
</acceptance_criteria>

<verification>
1. `cargo check -p maverick-runtime-edge` passes
2. Module structure is correct: `update::UpdateConfig`, `update::version`, `update::download`
3. `UpdateError` enum has all error variants
4. Version comparison handles: "1.1.0" vs "1.0.0", "1.0.0" vs "1.0.0", "2.0.0" vs "1.9.9"
5. `apply_release_update` creates backup before replacing binary
6. Atomic replace pattern: copy to .new, then rename (avoids partial write state)
</verification>

<success_criteria>
- Rust update module compiles and provides clean API
- Version checking works with HTTPS (valid cert) and HTTP (insecure mode)
- Download verifies file is non-empty before considering success
- Backup management keeps last 2 backups
- Error handling is comprehensive with thiserror::Error
</success_criteria>
